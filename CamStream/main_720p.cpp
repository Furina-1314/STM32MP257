 ...
 *   关键点: 传感器->CSI->input->ISP 全链保持 1080p(已验证的传感器模式,
 *   imx335 无 720p 原生模式的风险), 仅 dcmipp_main_postproc 的 compose
 *   设为 1280x720 由其硬件缩放, postproc 输出与采集节点为 720p
 *   /dev/video1 (DCMIPP, RGB888_1X24 1280x720)
...
#define SENSOR_WIDTH    1920              // 传感器->CSI->ISP 链路分辨率(保持1080p)
...
#define TARGET_FPS      30                // 探测失败时的兜底帧率
#define CAP_BUF_COUNT   4                 // V4L2 采集缓冲区个数
#define SDP_FILE        "stream_720p.sdp"
